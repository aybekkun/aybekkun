...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
	...
	return (
		<div
			className={`grid text-center gap-[4em] grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mx-auto py-[3em] overflow-visible ${
				className || ""
			}`}
		>
			{items.map((item, index) => (
				...
				>
					...
					<span
						className="absolute top-0 left-0 w-full h-full rounded-[1.25em] bg-white border-1 dark:bg-black/30  transition-[opacity,transform] duration-300 ease-[cubic-bezier(0.83,0,0.17,1)] origin-[80%_50%] flex backdrop-blur-[0.95em] [-webkit-backdrop-filter:blur(0.95em)] transform group-hover:[transform:translateZ(2em)]"
						style={{
